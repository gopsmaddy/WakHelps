...
public class TestExtJsReport2
{
    ...
    public void runReport()
    {
        ...
        try
        {
            ...
            //logger.debug("File Data:"+writer.toString());
            fu.writeFile("E:\\Wakkir\\workspaces\\intellij11\\AppHome\\output\\report","report3.xml",writer2.toString());

            StringWriter writer3 = new StringWriter();
            my_report_body_fo_tmpl.merge(context, writer3);
            //logger.debug("File Data:"+writer.toString());
            fu.writeFile("E:\\Wakkir\\workspaces\\intellij11\\AppHome\\output\\report","report3.fo",writer3.toString());

            File out = new File("E:\\Wakkir\\workspaces\\intellij11\\AppHome\\output\\report\\report3.pdf");
    ...
    String getDataValue(MyField field,int rowId)
    {
        ...
        {
            value=""+((field.getIndex()+1.0)*100.0/(field.getIndex()+rowId+2.0));
        }
    ...
    String getFormattedDataValue(String value,MyField field,int rowId)
    {
        ...
        else if("number".equalsIgnoreCase(field.getColumn().getDataType()))
        {
            ...
            if(field.getColumn().getFormat()!=null && field.getColumn().getFormat().length()>0)
                value=DecimalFormatter.getValue(field.getColumn().getFormat(),value,new Locale("en", "GB"));
        }
        else if("boolean".equalsIgnoreCase(field.getColumn().getDataType()))
        {

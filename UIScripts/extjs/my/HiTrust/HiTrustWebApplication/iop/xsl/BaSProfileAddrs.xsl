...
<xsl:output method="html" encoding="iso-8859-1" indent="no"/>
				...
				<xsl:copy-of select="//DST/password"/>			<!--<xsl:sort select="*[name() = $vsortKey]"/>-->				
 				...
 				<!--	
					...
					<xsl:choose>
							...
							<xsl:when test="$vsortKey='Address1'">
									<xsl:for-each select="//DST/response/List">											
										<xsl:sort select="./Address1"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>
							
							<xsl:when test="$vsortKey='Address3'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./Address3"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
							
							<xsl:when test="$vsortKey='State'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./State"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
							
							<xsl:when test="$vsortKey='ZipCode'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./ZipCode"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
							
							<xsl:when test="$vsortKey='Status'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./Status"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
							
							<xsl:when test="$vsortKey='DefaultAddr'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./DefaultAddr"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
 ...
 <xsl:template name="FilterData">		
		...
		<xsl:choose>
			<xsl:when test="
												(./Client/RelNo = $ProfileRelNo or $ProfileRelNo='') 	and
												(./Client/Key = $ProfileKey or $ProfileKey='') 			
										">
				<xsl:copy-of select="."/>	
			</xsl:when>
			<xsl:otherwise></xsl:otherwise>

...
<xsl:template name="UpdateNodes">	
	...
	<xsl:variable name="CorporateID"><xsl:value-of select="//DST/response/List/CorporateID" /></xsl:variable>
 	...
 	<List>	
		...
		<TFN>
			<xsl:choose>
				<xsl:when test="$IDTypeDesc ='TFN'">
					<xsl:choose>
						<xsl:when test="$TFN='' or $TFN='000000000'">Not Quoted</xsl:when>
						<xsl:otherwise>Quoted</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise><xsl:value-of select="$TFN"/></xsl:otherwise>
			</xsl:choose>
		</TFN>	
			
		<CorporateID>
			<xsl:choose>
				<xsl:when test="$CorporateID='' or $CorporateID='000000000'">Not Quoted</xsl:when>
				<xsl:otherwise><xsl:value-of select="$CorporateID"/></xsl:otherwise>
			</xsl:choose>
		</CorporateID>	
		
		<xsl:for-each select="./*">	
				
			<xsl:copy-of select="*[name()!='TFN']"/>	

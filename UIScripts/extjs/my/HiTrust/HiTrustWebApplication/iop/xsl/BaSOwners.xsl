...
-->						
					<xsl:choose>
							...
							<xsl:when test="$vsortKey='Profile_Salutation'">
									<xsl:for-each select="//DST/response/List">											
										<xsl:sort select="./Profile/Salutation"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>
							
							<xsl:when test="$vsortKey='Profile_GivenName'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./Profile/GivenName"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
							...
							<xsl:when test="$vsortKey='Profile_Name'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./Profile/Name"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
							
							<xsl:when test="$vsortKey='Profile_TFN'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./Profile/TFN"  order="{$vsortOrder}" data-type="{$vsortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
 ...
 <xsl:template name="FilterData">		
		...
		<xsl:choose>
			<xsl:when test="1=1"> <!-- Filter at wsml by portfolioKey -->
				<xsl:copy-of select="."/>	
			</xsl:when>
			<xsl:otherwise></xsl:otherwise>
		</xsl:choose>	

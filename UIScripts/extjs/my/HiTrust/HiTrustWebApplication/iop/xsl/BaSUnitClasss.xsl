...
-->
					...
					<xsl:choose>
							...
							<xsl:when test="$vsortKey='Key'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./Key"  order="{$vsortOrder}" data-type="{$sortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
							
							<xsl:when test="$vsortKey='Code'">
									<xsl:for-each select="//DST/response/List">										
										<xsl:sort select="./Code"  order="{$vsortOrder}" data-type="{$sortType}"/>
										<xsl:call-template name="FilterData"/>																													
									</xsl:for-each>
							</xsl:when>	
